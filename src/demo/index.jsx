...
const LuckRoll = forwardRef((props, ref) => {
      ...
      rollRef.current.querySelectorAll("ul").forEach((res, index) => {
        ...
        res.addEventListener(
          ...
          e => {
            ...
            e.target.style["-webkit-transform"] = `translate3d(0px, -${
              arr[index] * 100
            }%,0px)`;
            countNum++;
  ...
  useEffect(() => {
    let _li = new Array(6).fill("").map((res, index) => {
      return props.list.map((res2, index2) => {
  ...
  return (
    <div
      className="luckRollBox"
      ref={rollRef}
      style={{ width: props.width || "300px", height: props.height || "100px" }}
    >
      {new Array(props.cols * 1).fill("").map((res, index) => {
        return (
          <div
            className="luckRollitem"
            style={{ marginLeft: index != 0 ? `${props.space || "10px"}` : 0 }}
            key={index}
          >
            <ul
              style={{
                WebkitTransform: `translate3d(0,${
                  props.random ? rdmArr[index] : 0
                }%,0)`
              }}
            >
              {liDom}
